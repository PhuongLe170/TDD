...
{
    private const string PricingTableEndpoint = "PricingTable";
    ...
    public async Task Should_return_500_when_causes_exception()
    {
        ...
        var response = await
            client.PutAsJsonAsync(PricingTableEndpoint,
                CreateRequest());
    ...
    [Fact]
    public async Task Should_return_400_when_pricing_manager_returns_false()
    {
        using var client = CreateApiWithPricingManager<StubApplyFailPricingManager>()
            .CreateClient();
    ...
    {
        using var client =
            CreateApiWithPricingManager<StubApplySuccessPricingManager>()
                .CreateClient();

        var response = await client.PutAsJsonAsync(PricingTableEndpoint,
            CreateRequest());
    ...
    public async Task Should_send_request_to_pricing_manager()
    {
        ...
        var api = new ApiFactory(services => { services.TryAddScoped<IPricingStore>(s => pricingStore); });
        var client = api.CreateClient();
        var applyPricingRequest = CreateRequest();

        await client.PutAsJsonAsync(PricingTableEndpoint, applyPricingRequest);
